...
Ecoute.prototype.initializeSources = function (done) {
    ...
    return eachParallel.call(this.sources, this.config.sources, function (sources, key, done) {
        ...
        if (typeof sources[key].initialize === 'function') {
            ...
            return sources[key].initialize.call(this[sources[key].title], done);
        }
        else {
 ...
 *
 * @method initializeFileHandlers
 * @async
 * @param {Function} done callback
 */
Ecoute.prototype.initializeDataHandlers = function (done) {
    eachBatch(this.dataHandlers, function(dataHandler, key, done) {
        if (typeof dataHandler[key].initialize === 'function') {
            return dataHandler[key].initialize(done);
        }
        else {
            return done();
        }
...
Ecoute.prototype.runDataHandlers = function (done) {
    ...
    return eachSerial.call(this, sources, function (sources, key, done) {
        ...
        eachSerial.call(this, source, function (entries, key, done) {
            ...
            // create a 'processed' key on the entry, if it has none
            if (! ('processed' in data) || typeof data.processed !== {}) {
                data.processed = {};
            }
            ...
            eachBatch(this.dataHandlers, function (dataHandler, key, done) {
                ...
                if (dataHandler[key].match(data)) {
                    return dataHandler[key].execute(data, function(err, output) {
                        if (err) {
                            return done(err);
                        }

                        // returned value is stored in the entrys processed
                        // object in a key matching the name of the data-handler.
                        data.processed[dataHandler[key].type] = output;
